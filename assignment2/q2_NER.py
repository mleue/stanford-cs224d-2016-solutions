...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.variable_scope("embedding"):
        L = tf.get_variable("L", (len(self.wv), self.config.embed_size), initializer=xavier_weight_init())
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("softmax"):
      ...
      b2 = tf.Variable(tf.zeros(self.config.label_size), name='b2')

    hidden = tf.tanh(tf.matmul(window, W) + b1)
    h = tf.nn.dropout(hidden, self.dropout_placeholder)

    output = tf.matmul(h, U) + b2
    out = tf.nn.dropout(output, self.dropout_placeholder)

    ### END YOUR CODE
    return out 

  def add_loss_op(self, yhat):
    ...
    """
    ### YOUR CODE HERE
    loss_pred = tf.nn.softmax_cross_entropy_with_logits(yhat, self.labels_placeholder)
    with tf.variable_scope('hidden', reuse=True):
        W = tf.get_variable('W')
    with tf.variable_scope('softmax', reuse=True):
        U = tf.get_variable('U')
    loss_reg = (self.config.l2 / 2) * (tf.reduce_sum(tf.square(W)) + tf.reduce_sum(tf.square(U)))
    loss = tf.reduce_mean(loss_pred + loss_reg)
    ### END YOUR CODE
    return loss
